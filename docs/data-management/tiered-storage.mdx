...
- Enable Tiered Storage, either on a cluster or on a topic with a single command.
...
5. The sections below give detailed information on cloud storage configuration options, but the following properties are the minimum required properties that you must configure at the cluster level to use Tiered Storage for Amazon S3. Edit them with the `rpk cluster config edit` command.
...
5. The sections below give detailed information on cloud storage configuration options, but the following properties are the minimum required properties to use Tiered Storage for Google Cloud Storage. Edit them with the `rpk cluster config edit` command.
...
The table below gives the outcomes for possible combinations of cluster-level and topic-level configurations: 
...
Set retention limits on cloud storage to purge topic data after it reaches a specified age or size, or to purge data from local disks when they become full.
...
`retention.bytes`. A topic should not have ‘guaranteed’ local storage by default. Settings are applied only to the cloud storage, because it is presumed to be the primary tier when a topic is designated a remote read+write topic.   

You can specify hints on local storage when it's important to maintain low latency for a certain trailing window of data in that topic. 

The new `redpanda.retention.local` properties control the local retention of data.
...
The default value for `redpanda.topic.retention.local-target.hours` is 24 (1 day). When set, while balancing the demands of keeping recently-used data in local cache, the system targets to maintain this age of data in local storage. This purges data based on actual available local volume space, without forcing disk full situations when there is data skew.
