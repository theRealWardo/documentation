...
To maximize cloud storage, Redpanda supports retention limits and compaction. Set retention limits to purge topic data after it reaches a specified age or size.

Starting in Redpanda release 22.3, retention properties for Tiered Storage topics work the same as for local storage. Data is retained in the cloud until it reaches the configured time or size limit. The size limit applies to the total size of the topic whether the data is stored on disk or in the cloud.
