...
| `cloud_storage_enabled` | Global flag that enables Tiered Storage. Set to `true`. <br/><br/>Note that this property must be set to `true` to enable Tiered Storage at the cluster level or the topic level. Default is `false`. |
...
Data removal from the cloud (with retention limits and compaction).
Redpanda prevents compaction from processing any segments below `max_collectable_offset`. If archival is enabled for the partition, this means compaction can no longer prevent segment upload. 

1. Redpanda uploads non-compacted segments and propagates `last_uploaded_offset`.
2. Replace non-compacted segments by re-uploading compacted data. It uses its own offset variable `last_uploded_compacted_offset`. This replaces segments before they're deleted locally. 
3. During retention, before deleting data, check if data can be shrinked by re-uploading compacted segments. If so, start re-upload segments. 

Redpanda checks the size of all segments available for fetching after every upload. If the size exceeds the limit, it checks if it could re-upload compacted segments.

Set retention limits to purge topic data after it reaches a specified age or size, or to purge data from local disks when they become full.

Native Kafka retention controls apply to cloud storage. Cloud data is deleted when it expires or when topics are deleted. Redpanda caches actively used and sequential (next segment) data.

When a topic has cloud storage enabled, data expires from cloud storage following `retention.ms` or
`retention.bytes`. These properties control the total retention of data. For example, when `retention.bytes` is exceeded by data in S3, the data in S3 is trimmed.

For details about `retention.ms` and `retention.bytes`, see [Configure message retention](../../deployment/disk-utilization/#configure-message-retention).

two sets of parameters:
- Kafka style `retention.ms` and `retention.bytes`
  With Tiered Storage enabled, they set cloud retention limits per-partition, so if `retention.bytes` is set to 10GiB, then every partition in the topic should have 10GiB storage limit in the cloud.
- New redpanda configs `redpanda.local-target.ms` and `redpanda.local-target.bytes` for local data. These configs work per topic, so if size-based retention parameter is set to 10GiB, the value that the partition will have is 10GiB/number_of_partitions.
  The "local-target" parameters are equivalent to `retention.ms` and `retention.bytes` without Tiered Storage.

Topic creation

If Tiered Storage is enabled (cloud_storage_enabled=true, cloud_storage_remote_write=true, and cloud_storage_remote_read=true) or for the created topic (cloud_storage_enabled=true, redpanda.remote.write=true, and redpanda.remote.read=true), then:
- If `retention.ms` or `retention.bytes` is set, they initialize the remote_retention_ parameters in topic_properties.
- If `redpanda.local-target.ms` or `redpanda.local-target.bytes` are set, they are normalized and used to initialize the retention fields in topic_properties. Otherwise, use the default values for local retention parameters.
- If properties are not specified, use default values (infinite retention).

summary
- Total retention: The client-visible extent of data in a partition: i.e., what is retained and available to read by the client. For tiered storage topics, this is the combination of what’s in S3 and what’s in local storage, minus any overlap. For non-tiered-storage topics, this is simply the data on local disk.

- Local retention: The data retained on local storage in the Redpanda data directory. This does not include data in the tiered storage cache. Data in local storage may also be stored in S3, but will not be trimmed from local disk until the local retention limit is reached.

By default, a topic does not have guaranteed local storage. Settings are applied only to the cloud storage, because it is presumed to be the primary tier when a topic is designated a remote read or remote write topic. You can specify hints to stash a range of data in local storage. This is helpful when it's important to maintain low latency for a certain trailing window of data in that topic. 

The following properties control local data retention. Partition-level settings take priority. These settings evict data only when it would not contradict an explicit setting at the partition level. 
	
Partition-Level:
- `retention.local-target.hours`
- `retention.local-target.gb`

Topic-level:
- `redpanda.topic.retention.local-target.hours`
- `redpanda.topic.retention.local-target.gb`

The default value for `redpanda.topic.retention.local-target.hours` is 24 hours (one day). When set, the system keeps recently-used data in local cache and targets to maintain this age of data in local storage. It purges data based on actual available local volume space, without forcing disk full situations when there is data skew.

To ensure data eventually gets truncated after a reasonable period, the default value of `redpanda.topic.retention.hours` is seven days, but it can be set to other values without our cloud products.

**_confirm why this isn't listed under topic-level properties_**

By default, on topic creation, Redpanda enforces a one year retention for contracted FMC customers.
Pay-as-you go customers pay for whatever they store.

**_Link to separate section about upgrade. Existing topics have historical settings preserved on upgrade._**

Replace non-compacted segments with compacted ones to save space and minimize time needed to consume the entire topic. 
Replace small non-compacted segments with larger ones. 
1. Minimize replication
2. Minimize orphaned log segments in the cloud
