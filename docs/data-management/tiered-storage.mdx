...
The following sections provide detailed information on additional cloud storage configuration properties. 
...
| `cloud_storage_enabled` | Global flag that enables Tiered Storage. Set to `true`. Note that this property must be set to `true` to enable Tiered Storage at the cluster level or the topic level. Default is `false`. |
...
The following table gives the outcomes for possible combinations of cluster-level and topic-level configurations: 
...
The `retention.ms` and `retention.bytes` properties control the total retention of data. For example, when `retention.bytes` is exceeded by data in S3, the data in S3 is trimmed.

When a topic has cloud storage enabled, data expires from the cloud storage tier following `retention.ms` or
`retention.bytes`. By default, a topic does not have guaranteed local storage. Settings are applied only to the cloud storage, because it is presumed to be the primary tier when a topic is designated a remote read + write topic.   

You can specify hints to stash a range of data in local storage. This is helpful when it's important to maintain low latency for a certain trailing window of data in that topic. 

The following `redpanda.retention.local` properties control the local retention of data.
...
The default value for `redpanda.topic.retention.local-target.hours` is 24 (1 day). When set, while keeping recently-used data in local cache, the system targets to maintain this age of data in local storage. This purges data based on actual available local volume space, without forcing disk full situations when there is data skew.
...
For more information on `retention.ms` and `retention.bytes`, see [Configure message retention](../../deployment/disk-utilization/#configure-message-retention).
